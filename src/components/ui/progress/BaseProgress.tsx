...
export type BaseProgressProps = {
  ...
  additionalProgressOptions?: Omit<
    ProgressBarProps,
...
const BaseProgress = ({
  ...
  return (
    ...
    >
      <ProgressBar
        completed={currentProgressValue}
      ...
      >
        {startProgressLabel && (
          <p className={"label subtitle"}>{startProgressLabel}</p>
        )}
        {endProgressLabel && (
          <p className={"label subtitle"}>{endProgressLabel}</p>
        )}
      </div>
    </div>
  );
